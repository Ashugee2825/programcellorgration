...
<input type="hidden" name="page" value= "saveShowNextSpTrans">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=spTrans.getId()%>">
</form>

<form action="spTransCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextSpTrans">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=spTrans.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<SpTrans> it1= spTransList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="spTransCntrl?page=spTransDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="spTransCntrl?page=spTransDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="spTransCntrl?page=spTransDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="spTransCntrl?page=spTransDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	SpTrans spTrans1 = (SpTrans)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=spTrans1.getId()%></td>
<td><%=DateService.getDTSDDMMYYYY(spTrans1.getTransDt()).equals("11-11-1111")?"":DateService.getDTSDDMMYYYY(spTrans.getTransDt())%></td>
<td><%=spTrans1.getTransNos()%></td>
<td><%=spTrans1.getFeeAmt()%></td>
<td><a href="spTransCntrl?page=saveShowNextSpTrans&opr=edit&id=<%=spTrans1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="spTransCntrl?page=saveShowNextSpTrans&opr=delete&id=<%=spTrans1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
